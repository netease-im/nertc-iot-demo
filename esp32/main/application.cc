...
#if CONFIG_CONNECTION_TYPE_NERTC
    touch_timer_ = xTimerCreate("touch",
                                pdMS_TO_TICKS(2000),
                                pdFALSE,
                                this,
                                Application::TouchRestoreTimerCb);
    if (!touch_timer_) {
        ESP_LOGE(TAG, "Create touch_timer_ failed");
    }

    ring_timer_ = xTimerCreate("ring",
                                pdMS_TO_TICKS(4000),
                                pdTRUE,
                                this,
...
void Application::Start() {
    ...
    if (protocol_started) {
        ...
        audio_service_.PlaySound(Lang::Sounds::OGG_AGENT);
    }
}
...
#ifndef CONFIG_USE_NERTC_PTT_MODE
    SetDeviceState(kDeviceStateListening);
#else
    SetDeviceState(kDeviceStateIdle);
#endif
