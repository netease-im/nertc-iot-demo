...
#include "wake_words/nertc_afe_wake_word.h"
#else
...
void AudioService::ResetOpusParameters() {
    opus_frame_duration_ = OPUS_FRAME_DURATION_MS;
#ifdef CONFIG_CONNECTION_TYPE_NERTC
    ...
    max_send_packets_size_ = (960 / opus_frame_duration_);
#else
    max_send_packets_size_ = (600 / opus_frame_duration_);
#endif
    max_decode_packets_size_ = (600 / opus_frame_duration_);
...
#endif
    codec_ = codec;
    codec_->SetOpusFrameDuration(opus_frame_duration());
    codec_->Start();
...
void AudioService::Start() {
    ...
    xTaskCreate([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "wake_opus_codec", 8 * 1024, this, 2, &wake_opus_codec_task_handle_);
#endif
...
#ifdef CONFIG_USE_AUDIO_CODEC_ENCODE_OPUS
void AudioService::OnAudioInputDecodeForWakeWord() {
    std::vector<uint8_t> data;
    int samples = wake_word_->GetFeedSize();
    if (samples > 0) {
        if (ReadAudioData(data, 16000, 20 * 16000 / 1000)) {
                if(opus_decoder2_ == nullptr || wake_word_ == nullptr || !wake_word_initialized_ || service_stopped_){
                    return;
                }
                std::lock_guard<std::mutex> lock(wake_audio_queue_mutex_);
                if(wake_word_opus_queue_.size() < 10)
                {
                    wake_word_opus_queue_.push_back(std::move(data));
                }
                wake_audio_queue_cv_.notify_all();
        }
        {
            std::unique_lock<std::mutex> lock(wake_wake_pcm_buffer_mutex_);
            if(wake_pcm_buffer_.size() >= samples){
                std::vector<int16_t> pcm(wake_pcm_buffer_.begin(), wake_pcm_buffer_.begin() + samples);
                wake_pcm_buffer_.erase(wake_pcm_buffer_.begin(), wake_pcm_buffer_.begin() + samples);
                lock.unlock();
                wake_word_->Feed(pcm);
                return;
            }
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        if (bits & AS_EVENT_AUDIO_TESTING_RUNNING) {
            if (audio_testing_queue_.size() >= AUDIO_TESTING_MAX_DURATION_MS / opus_frame_duration()) {
    ...
    while (true) {
        ...
        audio_queue_cv_.wait(lock, [this]() {
            return service_stopped_ ||
                (!audio_encode_queue_.empty() && audio_send_queue_.size() < max_send_packets_size_) ||
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            if (opus_decoder_->Decode(std::move(packet->payload), task->pcm)) {
                ...
                ESP_LOGE(TAG, "Failed to decode audio, packet.payload size:%d", payload_size);
                lock.lock();
            }
        ...
        if (!audio_encode_queue_.empty() && audio_send_queue_.size() < max_send_packets_size_) {
            while(audio_encode_queue_.size() > MAX_ENCODE_TASKS_IN_QUEUE) {
                ESP_LOGW(TAG, "Audio encode queue is full (%u), dropping oldest task", audio_encode_queue_.size());
...
bool AudioService::PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    if (audio_decode_queue_.size() >= max_decode_packets_size_) {
        if (wait) {
            audio_queue_cv_.wait(lock, [this]() { return audio_decode_queue_.size() < max_decode_packets_size_; });
        } else {
            return false;
        }
    }
...
#ifdef CONFIG_USE_AUDIO_CODEC_ENCODE_OPUS
        {
            wake_audio_queue_mutex_.lock();
            wake_word_opus_queue_.clear();
            wake_audio_queue_mutex_.unlock();
            wake_wake_pcm_buffer_mutex_.lock();
            wake_pcm_buffer_.clear();
            wake_wake_pcm_buffer_mutex_.unlock();
        }
#endif
...
#if (defined(CONFIG_USE_AUDIO_CODEC_ENCODE_OPUS) || defined(CONFIG_USE_AUDIO_CODEC_DECODE_OPUS))
    ESP_LOGW(TAG, "Codec not support audio testing");
#else
...
#ifdef CONFIG_USE_AUDIO_CODEC_ENCODE_OPUS
void AudioService::WakeOpusCodecTask() {
    while (true) {
        {
            std::unique_lock<std::mutex> lock(wake_audio_queue_mutex_);
            wake_audio_queue_cv_.wait(lock, [this]() {
                return !service_stopped_ && wake_word_opus_queue_.size() > 0;
            });
        }   
        if (!wake_word_opus_queue_.empty()) {
            std::unique_lock<std::mutex> lock(wake_audio_queue_mutex_);
            auto data = std::move(wake_word_opus_queue_.front());
            wake_word_opus_queue_.pop_front();
            wake_audio_queue_cv_.notify_all();
            lock.unlock();
            std::vector<int16_t> pcm;
            if (opus_decoder2_->Decode(std::move(data), pcm)) {
                std::unique_lock<std::mutex> lock(wake_wake_pcm_buffer_mutex_);
                wake_pcm_buffer_.insert(wake_pcm_buffer_.end(), pcm.begin(), pcm.end());
                lock.unlock();
            }
        }
    }
