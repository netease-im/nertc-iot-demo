...
void Application::ActivationTask() {
    ...
    auto& board = Board::GetInstance();
    auto codec = board.GetAudioCodec();
    if (ota_->GetSupportAirMusicPlayer() && (Board::GetInstance().GetBoardType() != "ml307" || ota_->GetSupportAirMusicIn4G())){
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnAudioChannelClosed([this, &board]() {
        ...
        Schedule([this]() {
            ...
            SetDeviceState(kDeviceStateIdle);
            Schedule([this]() {
                Board::GetInstance().GetDisplay()->SetEmotion("sleepy");
            });
        });
    });
...
#if CONFIG_CONNECTION_TYPE_NERTC
            // 处理服务端 agentEvent 下发的闹钟列表
            auto alarms_obj = cJSON_GetObjectItem(root, "alarms");
            if (cJSON_IsArray(alarms_obj)) {
                int alarm_count = cJSON_GetArraySize(alarms_obj);
                if (alarm_manager_) {
                    alarm_manager_->ClearAll();
                }
                for (int i = 0; i < alarm_count; ++i) {
                    auto alarm_obj = cJSON_GetArrayItem(alarms_obj, i);
                    ESP_LOGI(TAG, "Alarm: %s", cJSON_Print(alarm_obj));
                    Alarm alarm;
                    alarm.name = cJSON_GetObjectItem(alarm_obj, "name")->valuestring;
                    alarm.current_ts = cJSON_GetObjectItem(alarm_obj, "currentTs")->valueint;
                    alarm.next_trigger_ts = cJSON_GetObjectItem(alarm_obj, "nextTriggerTs")->valueint;
                    alarm.id = cJSON_GetObjectItem(alarm_obj, "id")->valueint;
                    alarm.status = cJSON_GetObjectItem(alarm_obj, "status")->valueint;
                    alarm.alarm_type = (AlarmType)cJSON_GetObjectItem(alarm_obj, "alarmType")->valueint;
                    alarm.cron_expression = cJSON_GetObjectItem(alarm_obj, "cronExpression")->valuestring;
                    if (alarm_manager_) {
                        alarm_manager_->AddAlarm(alarm);
                    }
                }
            } else {
                ESP_LOGW(TAG, "Alarm command requires alarms");
            }
#endif
        } else {
