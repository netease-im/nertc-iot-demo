...
#define TAG "Esp32CameraLegacy"

Esp32CameraLegacy::Esp32CameraLegacy(const camera_config_t& config) {
    // camera init
...
Esp32CameraLegacy::~Esp32CameraLegacy() {
    if (fb_) {
...
void Esp32CameraLegacy::SetExplainUrl(const std::string& url, const std::string& token) {
    explain_url_ = url;
    explain_token_ = token;
}

bool Esp32CameraLegacy::Capture() {
    ...
    if (display != nullptr && fb_->buf != nullptr && fb_->len > 0) {
        uint16_t w = fb_->width;
        uint16_t h = fb_->height;
        size_t data_size = fb_->len;
        ...
        if (data != nullptr) {
            ...
            auto image = std::make_unique<LvglAllocatedImage>(
                data, data_size, w, h, stride, LV_COLOR_FORMAT_RGB565);
...
bool Esp32CameraLegacy::SetHMirror(bool enabled) {
...
bool Esp32CameraLegacy::SetVFlip(bool enabled) {
    sensor_t *s = esp_camera_sensor_get();
    if (s == nullptr) {
        ESP_LOGE(TAG, "Failed to get camera sensor");
        return false;
 ...
 */
std::string Esp32CameraLegacy::Explain(const std::string& question) {
...
bool Esp32CameraLegacy::GetCapturedJpeg(uint8_t*& data, size_t& len) {
