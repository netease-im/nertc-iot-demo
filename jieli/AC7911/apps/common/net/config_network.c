...
#if BT_NET_CFG_DUI_EN || BT_NET_CFG_TURING_EN || BT_NET_CFG_TENCENT_EN || BT_NET_CFG_QYAI_EN || BT_NET_CFG_GIZWITS_EN
#undef  BT_NET_CFG_EN
...
#if TCFG_USER_BLE_ENABLE && BT_NET_CFG_EN
#if BT_NET_CFG_GIZWITS_EN

void gizwits_net_config(const u8 *ssid, const u8 ssid_len, const u8 *pwd, const u8 pwd_len)
{
    memcpy(bt_net_result.ssid, ssid, ssid_len);
    memcpy(bt_net_result.pwd, pwd, pwd_len);
    printf("gizwits net config ssid : %s", bt_net_result.ssid);
    printf("gizwits net config pwd : %s", bt_net_result.pwd);
}
