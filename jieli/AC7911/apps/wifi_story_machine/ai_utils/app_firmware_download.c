...
{
// void app_music_play_update_state(u8 state);  
//     app_music_play_update_state(1);
...
static char http_download_ur[200]={0};

static int http_create_download_task(void)
{
    printf("http_create_download_task\n");
    struct download_parm parm = {0};
    struct download_hdl *hdl = NULL;

    parm.url = http_download_ur;//OTA_URL_ADDR;
...
void my_app_firware_download(char *url) {
    ...
    memset(http_download_ur,0,sizeof(http_download_ur));
    strcpy(http_download_ur,url);
