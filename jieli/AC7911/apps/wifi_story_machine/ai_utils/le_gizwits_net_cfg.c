...
#if 1//(TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_NET_CFG_GIZWITS)

#include "le_gizwits_profile.h"


#define TEST_SEND_HANDLE_VAL         ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    printf("read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);
    ...
    //     att_value_len = 2;
    //     break;
    case ATT_CHARACTERISTIC_ABD4_VALUE_HANDLE:
    printf("ATT_CHARACTERISTIC_ABD4_VALUE_HANDLE");
    break;

    case ATT_CHARACTERISTIC_ABD5_VALUE_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD5_VALUE_HANDLE");
        break;

    case ATT_CHARACTERISTIC_ABD6_VALUE_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD6_VALUE_HANDLE");
        // att_value_len = sizeof(indicate_value);
        // if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len) {
        //     break;
        // }

        // if (buffer) {
        //     memcpy(buffer, &indicate_value[offset], buffer_size);
        //     att_value_len = buffer_size;
        //     printf("read value: %s", indicate_value);
        // }
        break;

    case ATT_CHARACTERISTIC_ABD7_VALUE_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD7_VALUE_HANDLE");
        break;

    case ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE");
        // att_value_len = sizeof(notify_value);
        // if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len) {
        //     break;
        // }

        // if (buffer) {
        //     memcpy(buffer, &notify_value[offset], buffer_size);
        //     att_value_len = buffer_size;
        //     printf("read value: %s", notify_value);
        // }
        break;
    ...
    if (user_buf[user_buf_offset - 1] == 0xFF && user_buf_offset == user_data_size + 7) {
        ...
        if (crc16 == CRC16(&user_buf[2], user_data_size + 2)) {
            ...
            if (0 == bt_net_config_set_ssid_pwd((const char *)&user_buf[6])) {
                printf("SSSSSSSSSSS--------------------------SSSSSSSSSSS");
                app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, rsp_cmd1, sizeof(rsp_cmd1), ATT_OP_AUTO_READ_CCC);
                complete = 1;
                user_buf_offset = 0;
                user_data_size = 0;
            ...
            printf("SSSSSSSSSSS-==============================SSSSSSSSSSS");
            app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, rsp_cmd2, sizeof(rsp_cmd2), ATT_OP_AUTO_READ_CCC);
            user_buf_offset = 0;
            user_data_size = 0;
            memset(user_buf, 0, sizeof(user_buf));
        }
...
static int payment_msg_send(void *priv)
{
    ...
    printf("SSSSSSSSSSS++++++++++++++++++++++SSSSSSSSSSS");
    app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, msg_cmd, len + 7, ATT_OP_AUTO_READ_CCC);
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    uint8_t response[21];
    int ret = 0;
    printf("write_callback, handle= 0x%04x,size = %d\n", handle, buffer_size);
    ...
    case ATT_CHARACTERISTIC_ABD5_VALUE_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD5_VALUE_HANDLE: %d | %d", buffer_size, user_buf_offset);
        put_buf(buffer, buffer_size);

        if (user_buf_offset + buffer_size > sizeof(user_buf)) {
            log_error("ATT_CHARACTERISTIC_ABD5_VALUE_HANDLE: buffer overflow");
            user_buf_offset = 0;
            user_data_size = 0;
            memset(user_buf, 0, sizeof(user_buf));
            // @TODO: 发送失败消息给手机端
        ...
        protocol_data_t result = protocol_parse_data(user_buf, user_buf_offset);
        if (result.success) {
            printf("protocol_parse_data success: %d", result.cmd);
            switch (result.cmd) {
            case CMD_WIFI_CONFIG:
                printf("CMD_WIFI_CONFIG");
                wifi_config_t *wifi_config = &result.data.wifi_config;
                // 设置wifi名字和密码给配网程序
                gizwits_net_config(wifi_config->ssid, wifi_config->ssid_len,wifi_config->password, wifi_config->password_len);
                wifi_prov_set_result(wifi_config->ssid, wifi_config->ssid_len,wifi_config->password, wifi_config->password_len, wifi_config->uid, wifi_config->uid_len);

                // @TODO: 处理 UID

                size_t resp_len = pack_wifi_config_response(0, result.msg_id,RESP_STATUS_OK, response, sizeof(response));
                if (resp_len != sizeof(response)) {
                    log_error("pack_wifi_config_response failed");
                    break;
                }
                printf("vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv");
                ret = app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE,response, resp_len, ATT_OP_AUTO_READ_CCC);
                
                if (ret) {
                    log_error("app_send_user_data failed: %d", ret);
                }
                else {
                    printf("app_send_user_data success");
                ...
                break;
            default:
                log_error("Unknown CMD: %d", result.cmd);
                break;
            }
        }
        else {
            log_error("protocol_parse_data failed");
        }
    ...
    case ATT_CHARACTERISTIC_ABD6_CCC_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD6_CCC_HANDLE: %04x, %02x", handle, buffer[0]);
        set_ble_work_state(BLE_ST_NOTIFY_IDICATE);
        check_connetion_updata_deal();
        att_set_ccc_config(handle, buffer[0]);
        user_buf_offset = 0;
    ...
    case ATT_CHARACTERISTIC_ABD8_CCC_HANDLE:
        printf("ATT_CHARACTERISTIC_ABD8_CCC_HANDLE: %04x, %02x", handle, buffer[0]);
        set_ble_work_state(BLE_ST_NOTIFY_IDICATE);
        check_connetion_updata_deal();
        att_set_ccc_config(handle, buffer[0]);
        user_buf_offset = 0;
...
static int make_set_adv_data(void)
{
    ...
    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_COMPLETE_16BIT_SERVICE_UUIDS, GIZ_BLE_SERVICE_UUID, 2);

    // Define the function mask
    u8 func_mask = 0;
    func_mask = BLE_FUNC_SET_VERSION(func_mask, BLE_VERSION_5_0);
    func_mask = BLE_FUNC_ENABLE_OTA(func_mask);
    func_mask = BLE_FUNC_SET_UNIQUE_KEY(func_mask);

    // Manufacturer specific data structure
    u8 manu_data[14] = {
        manu_id[0], manu_id[1],         // Company ID LSB (0x3D)
        BLE_MAKE_VER_TYPE(BLE_VERSION_5, BLE_DEVICE_TYPE_GATT), // 0xB5
        func_mask,                      // Function Mask
        pk[0], pk[1], pk[2], pk[3],     // PK (4 bytes)
    };
    
    // Get MAC address and append it to manu_data
    const u8 *mac_addr = bt_get_mac_addr();
    memcpy(&manu_data[8], mac_addr, 6);  // Copy 6 bytes MAC address
    
    offset += make_eir_packet_data(&buf[offset], offset,
            HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA,
            manu_data, sizeof(manu_data));
...
void ble_profile_init(void)
{
    ...
    /* setup ATT server */
    att_server_init(giz_profile_data, att_read_callback, att_write_callback);
...
#endif
printf("uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu");
    return app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
...
{
    printf("----------------------55----------------------------------------ble_app_disconnect");
    ble_disconnect(NULL);
}

void ble_cfg_net_result_notify(int event)
{
    printf("---------------22----------------------ble_cfg_net_result_notify");
    if(!con_handle)return;
    // if (!complete)return;

    if (event == NET_CONNECT_TIMEOUT_NOT_FOUND_SSID) {
        printf("SSSSSSSSS---NET_CONNECT_TIMEOUT_NOT_FOUND_SSID");
        app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, rsp_cmd3, sizeof(rsp_cmd3), ATT_OP_AUTO_READ_CCC);
    } else if (event == NET_CONNECT_ASSOCIAT_FAIL) {
        printf("SSSSSSSSS---NET_CONNECT_ASSOCIAT_FAIL");
        app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, rsp_cmd4, sizeof(rsp_cmd4), ATT_OP_AUTO_READ_CCC);
    } else if (event == NET_EVENT_CONNECTED) {
        printf("------------------33-------------------ble_cfg_net_result_notify");
        app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, rsp_cmd1, sizeof(rsp_cmd1), ATT_OP_AUTO_READ_CCC);
        printf("------------------44-------------------ble_cfg_net_result_notify");
        app_send_user_data(ATT_CHARACTERISTIC_ABD8_VALUE_HANDLE, rsp_cmd0, sizeof(rsp_cmd0), ATT_OP_AUTO_READ_CCC);
        complete = 0;
#if !defined PAYMENT_AUDIO_SDK_ENABLE && !CONFIG_POWER_ON_ENABLE_BLE
        sys_timeout_add(0, (void (*)(void *))ble_module_enable, 10000);
#endif
    }
}
