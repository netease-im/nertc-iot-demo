...
extern int nertc_app_event_init(void);

#define TAG "NERTC_APP"
#define WIFI_STARTUP_DELAY_MS 2000
...
{
#if (CONFIG_SYS_CPU0)
    BK_LOGW(TAG, "=== [NERTC] user_app_main start ===\n");
    
    BK_LOGW(TAG, "[NERTC] Setting CPU frequency to 240MHz\n");
    bk_pm_module_vote_cpu_freq(PM_DEV_ID_AUDIO, PM_CPU_FRQ_240M);

    BK_LOGW(TAG, "[NERTC] Initializing audio engine\n");
    audio_engine_init();
    
    BK_LOGW(TAG, "[NERTC] Initializing video engine\n");
    voide_engine_init();
    
    BK_LOGW(TAG, "[NERTC] Initializing network transfer\n");
    network_transfer_init();

    BK_LOGW(TAG, "[NERTC] Registering NERTC CLI\n");
    nertc_cli_init();
    
    BK_LOGW(TAG, "=== [NERTC] user_app_main complete ===\n");
#endif

#if (CONFIG_SYS_CPU1)
    BK_LOGW(TAG, "CPU1: Initializing video engine\n");
    voide_engine_init();
#endif
}
...
#endif
#if CONFIG_BK_SMART_CONFIG
        BK_LOGW(TAG, "[NERTC] Delay %d ms before wifi startup\n", WIFI_STARTUP_DELAY_MS);
        rtos_delay_milliseconds(WIFI_STARTUP_DELAY_MS);
